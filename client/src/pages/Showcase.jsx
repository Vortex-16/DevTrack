                ...
                <div className="p-5">
                    ...
                    <div className="flex items-center gap-2 mb-3">
                        {showcase.ownerAvatar ? (
                            <img src={showcase.ownerAvatar} alt={showcase.ownerName} className="w-6 h-6 rounded-full" />
                        ) : (
                    ...
                    {/* Title & Summary */}
                    <h3 className="text-lg font-bold text-white mb-2 line-clamp-1">{showcase.projectName}</h3>
                    <p className="text-slate-400 text-sm mb-4 line-clamp-2">{showcase.summary}</p>
                        ...
                        <div className="flex items-center gap-4">
                            <button
                                ...
                                className={`flex items-center gap-1.5 text-sm transition-colors ${
                                    showcase.hasStarred ? 'text-yellow-400' : 'text-slate-400 hover:text-yellow-400'
                                }`}
                            >
            ...
            <AnimatePresence mode="wait">
                ...
                {step === 2 && (
                    ...
                    >
                        ...
                        {/* Summary */}
                        <div>
                            <label className="block text-sm text-slate-400 mb-2">Project Summary *</label>
                            <textarea
                                ...
                                maxLength={500}
                            />
                            <p className="text-slate-500 text-xs mt-1 text-right">{formData.summary.length}/500</p>
                        </div>
                        ...
                        {/* Template Selection */}
                        <div>
                            <label className="block text-sm text-slate-400 mb-2">Project Category</label>
                        ...
                        {/* Image Upload */}
                        <div>
                            <label className="block text-sm text-slate-400 mb-2">Showcase Image (optional)</label>
                        ...
                        <div className="p-4 rounded-xl bg-emerald-500/5 border border-emerald-500/10">
                            <div className="flex items-center justify-between mb-3">
                                <div className="flex items-center gap-2">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-[10px] text-slate-500">
                                            Enable this to find collaborators for this project
                                        </p>
                                    </div>
                                </div>
                                <button
                                ...
                                >
                                    <div
                                        className={`absolute top-1 w-3 h-3 rounded-full bg-white transition-all ${formData.openToBuild ? 'right-1' : 'left-1'}`}
                                    />
                                </button>
                            </div>
...
export default function Showcase() {
    ...
    const filteredShowcases = showcases.filter((s) => {
        ...
        const matchesTech =
            !techFilter || s.technologies?.some((t) => t.toLowerCase().includes(techFilter.toLowerCase()));
    ...
    const availableProjects = publicProjects.filter((p) => !myShowcases.some((s) => s.projectId === p.id));
    ...
    const handleToggleOpenToBuild = async (showcaseId, currentValue) => {
        ...
        setMyShowcases((prev) => prev.map((s) => (s.id === showcaseId ? { ...s, openToBuild: newValue } : s)));
        try {
            await showcaseApi.toggleOpenToBuild(showcaseId, newValue);
        } catch (err) {
            console.error('Failed to toggle Open to Build:', err);
            // Revert on failure
            setMyShowcases((prev) => prev.map((s) => (s.id === showcaseId ? { ...s, openToBuild: currentValue } : s)));
        }
    };

    return (
        <div id="showcase-scroll-container" className="min-h-screen bg-slate-950 overflow-y-auto">
            <div className="px-4 md:px-6 py-0">
                {/* Header */}
                <motion.div
                    initial={{ opacity: 0, y: -20 }}
                    animate={{ opacity: 1, y: 0 }}
                    className="mb-2 md:mb-4 mt-2 md:mt-0"
                >
                    <div className="flex items-center gap-3 mb-2">
                        <div>
                            <h1 className="text-3xl font-bold text-white">Project Showcase</h1>
                            <p className="text-slate-400 text-sm">Discover amazing projects from the community</p>
                        </div>
                    </div>
                </motion.div>
                ...
                <div className="flex gap-2 mb-6 bg-white/5 p-1 rounded-xl w-full sm:w-fit">
                    <button
                        ...
                        className={`flex-1 sm:flex-none flex items-center justify-center whitespace-nowrap px-3 py-1.5 md:px-4 md:py-2 rounded-lg text-xs md:text-sm font-medium transition-colors ${
                            activeTab === 'discover' ? 'bg-purple-500 text-white' : 'text-slate-400 hover:text-white'
                        }`}
                    >
                    ...
                    <button
                        ...
                        className={`flex-1 sm:flex-none flex items-center justify-center whitespace-nowrap px-3 py-1.5 md:px-4 md:py-2 rounded-lg text-xs md:text-sm font-medium transition-colors ${
                            activeTab === 'mine' ? 'bg-purple-500 text-white' : 'text-slate-400 hover:text-white'
                        }`}
                    >
                                            ...
                                            return (
                                                <div className="col-span-full py-20 text-center bg-white/5 rounded-3xl border border-dashed border-white/10">
                                                    ...
                                                    <h3 className="text-white font-bold mb-2">
                                                        No active builders found
                                                    </h3>
                                                    <p className="text-slate-500 text-sm">
                                                        Check back later or enable "Open to Build" on your profile!
                                                    </p>
                                                </div>
                                            );
                                        }
                                            ...
                                            >
                                                <div className="flex items-center gap-4 mb-6">
                                                    <img
                                                        src={
                                                            builder.ownerAvatar ||
                                                            `https://ui-avatars.com/api/?name=${builder.ownerName}`
                                                        }
                                                    ...
                                                    <div>
                                                        ...
                                                        <Link
                                                            to={`/public/profile/${builder.ownerGithub || builder.ownerName}`}
                                                            className="text-emerald-500/80 text-xs hover:underline flex items-center gap-1"
                                                        >
                                                ...
                                                <div className="bg-emerald-500/5 rounded-2xl p-4 border border-emerald-500/10 mb-6">
                                                    ...
                                                    <p className="text-slate-300 text-sm italic leading-relaxed">
                                                        "
                                                        {builder.collaboration?.goal ||
                                                            'Looking for partners to build something epic.'}
                                                        "
                                                    </p>
                                                </div>
                                                ...
                                                <div>
                                                    ...
                                                    <div className="flex flex-wrap gap-2">
                                                        {(builder.collaboration?.seekingStack || []).map((skill, i) => (
                                                            <span
                                                                key={i}
                                                                className="px-2.5 py-1 rounded-lg bg-emerald-500/10 text-emerald-400 text-[10px] font-bold border border-emerald-500/20"
                                                            >
                                                                {skill}
                                                            </span>
                                                        ))}
                                ...
                                {availableProjects.length === 0 && myProjects.length > 0 && (
                                    <div className="mb-6 p-4 rounded-xl bg-amber-500/10 border border-amber-500/20">
                                        <p className="text-amber-300 text-sm flex items-center gap-2">
                                            ...
                                            <span>
                                                Only <strong>public GitHub repositories</strong> from your Projects can
                                                be showcased.
                                        ...
                                        {availableProjects.length > 0 ? (
                                            ...
                                            <div className="space-y-2">
                                                <p className="text-slate-500 text-sm">
                                                    ...
                                                    {myProjects.length > 1 ? 's' : ''}, but none are eligible for
                                                    showcase.
                                                </p>
                                                <p className="text-slate-500 text-sm">
                                                    Only{' '}
                                        ...
                                        {myShowcases.map((showcase) => (
                                            <div key={showcase.id} className="relative">
                                                ...
                                                <div className="absolute top-3 right-3 z-10 flex gap-2">
                                                    ...
                                                    <button
                                                        onClick={() =>
                                                            handleToggleOpenToBuild(showcase.id, showcase.openToBuild)
                                                        }
                                                        ...
                                                        title={
                                                            showcase.openToBuild
                                                                ? 'Click to disable Open to Build'
                                                                : 'Click to mark as Open to Build'
                                                        }
                                                    >
                                                        <ShieldCheck size={10} />
