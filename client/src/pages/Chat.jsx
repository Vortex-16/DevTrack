    ...
    return (
        <ReactMarkdown
            components={{
                ...
                h3: ({ children }) => <h3 className="text-md font-semibold text-purple-300 mt-2 mb-1">{children}</h3>,
                ...
                pre: ({ children }) => (
                    <pre className="bg-white/5 text-emerald-400 p-3 rounded-xl my-2 overflow-x-auto text-sm border border-white/10">
                        {children}
                    </pre>
                ),
                code: ({ inline, className, children, ...props }) => {
                    ...
                    if (inline) {
                        return (
                            <code className="bg-white/10 text-emerald-400 px-1.5 py-0.5 rounded text-sm" {...props}>
                                {children}
                            </code>
                        );
                    }
...
function MessageBubble({ message, idx }) {
    ...
    return (
        ...
        >
            <div
                className={`flex items-start gap-3 w-full sm:max-w-[85%] sm:w-auto ${isUser ? 'flex-row-reverse' : ''}`}
            >
                {/* Avatar */}
                <div
                    className={`flex-shrink-0 w-8 h-8 rounded-xl overflow-hidden flex items-center justify-center
                    ${
                        isUser
                ...
                {/* Message Content */}
                <div
                    className={`relative px-4 py-2 shadow-sm max-w-[calc(100%-3rem)] ${
                        isUser
...
export default function Chat() {
    ...
    const quickPrompts = [
        {
            icon: <Target className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-purple-400" />,
            label: 'Which project first?',
            prompt: 'Based on my projects, which one should I focus on first and why?',
        },
        {
            icon: <BookOpen className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-blue-400" />,
            label: 'What to learn?',
            prompt: 'What should I learn next to improve my skills?',
        },
        {
            icon: <Dumbbell className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-emerald-400" />,
            label: 'Motivate me',
            prompt: 'Give me a motivational message based on my progress.',
        },
        {
            icon: <BarChart2 className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-orange-400" />,
            label: 'Analyze progress',
            prompt: 'Analyze my overall progress. What can I improve?',
        },
    ];

    return (
        <PixelTransition loading={loading && messages.length === 0}>
            <motion.div className="h-[calc(100vh-4rem)] flex gap-4 lg:gap-6 overflow-hidden relative">
                {/* Loader Overlay removed - replaced by PixelTransition */}

                {/* Sidebar Removed */}

                {/* Main Chat Container - Background removed, padding maximized */}
                <div className="px-4 md:px-6 py-0 flex-1 flex flex-col overflow-hidden relative">
                    ...
                    <form onSubmit={handleSubmit} className="flex gap-3">
                        ...
                        <Button type="submit" disabled={loading || !input.trim()} className="px-6 rounded-full">
                            {loading ? (
                                <motion.div
                                    animate={{
                                        x: [0, 15, -15, 0],
