...
class ReportService {
    ...
    drawPage1ExecutiveDashboard(doc, data, aiInsights, subtitle) {
        ...
        doc.font('Helvetica').fontSize(10).fillColor(this.colors.primaryDark).text(dash.biggestWin || 'No major win recorded.', 65, y + 35, { width: doc.page.width - 130 });
        y += 90;
        
        const cardW = (doc.page.width - 110) / 2;
        this.drawCard(doc, 50, y, cardW, 70, 'Primary Risk');
        doc.font('Helvetica').fontSize(9).fillColor(this.colors.danger).text(dash.biggestRisk || 'No critical risks detected.', 65, y + 35, { width: cardW - 30 });
        this.drawCard(doc, 50 + cardW + 10, y, cardW, 70, 'Strategic Focus');
        doc.font('Helvetica').fontSize(9).fillColor(this.colors.accent).text(dash.strategicFocus || 'Maintain current trajectory.', 65 + cardW + 10, y + 35, { width: cardW - 30 });
    }
    ...
    drawPage2PerformanceIntelligence(doc, aiInsights, subtitle) {
        ...
        if (behavioral.behaviorSummary) {
            ...
            doc.font('Helvetica').fontSize(9).fillColor(this.colors.textLight).text(behavioral.behaviorSummary, 65, y + 30, { width: doc.page.width - 130 });
            y += 70;
        }
    ...
    drawPage6WeeklyNarrative(doc, aiInsights, data, subtitle) {
        ...
        doc.font('Helvetica').fontSize(10).fillColor(this.colors.textLight).text(narrative, 65, y + 44, { width: doc.page.width - 130, lineGap: 5 });
        y += 178;
