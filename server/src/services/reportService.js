...
class ReportService {
    ...
    tint(hex, amount = 0.12) {
        return this.mix(this.colors.white, hex, amount);
    }
    ...
    _triangle(doc, cx, cy, size, dir, color) {
        if (dir === 'flat') {
            doc.rect(cx - size, cy - 1, size * 2, 2).fill(color);
            return;
        }
        const up = dir === 'up';
    ...
    _glyph(doc, kind, cx, cy, s, color) {
        switch (kind) {
            ...
            case 'bolt': {
                const w = s * 1.1,
                    h = s * 1.7;
                doc.moveTo(cx - w * 0.08, cy - h / 2)
                    .lineTo(cx - w / 2, cy + h * 0.1)
                    .lineTo(cx - w * 0.05, cy + h * 0.1)
                    .lineTo(cx + w * 0.08, cy + h / 2)
                    .lineTo(cx + w / 2, cy - h * 0.1)
                    .lineTo(cx + w * 0.05, cy - h * 0.1)
                    .closePath()
                    .fill(color);
    ...
    drawBadgeMedal(doc, cx, cy, r, color, kind) {
        // soft shadow
        doc.circle(cx, cy + 1.5, r)
            .fillOpacity(0.12)
            .fill(this.colors.ink)
            .fillOpacity(1);
        // ribbon tails (behind coin)
        const ribbon = this.mix(color, this.colors.ink, 0.3);
        doc.moveTo(cx - r * 0.5, cy + r * 0.3)
            .lineTo(cx - r * 0.72, cy + r * 1.7)
            .lineTo(cx - r * 0.15, cy + r * 1.25)
        ...
        doc.circle(cx, cy, r)
            .lineWidth(1.5)
            .strokeColor(this.mix(color, this.colors.ink, 0.3))
            .stroke();
        doc.circle(cx, cy, r * 0.74)
            .lineWidth(0.8)
            .strokeColor(this.mix(color, this.colors.white, 0.45))
            .stroke();
        // glyph
    ...
    badgeStyle(b) {
        ...
        if (id.includes('pr') || name.includes('pr ') || name.includes('master'))
            return { kind: 'bolt', color: '#3B82F6' };
        if (id.includes('community') || name.includes('leader') || name.includes('community'))
            return { kind: 'star', color: '#8B5CF6' };
    ...
    drawGradientHeader(doc, title, subtitle, accent) {
        ...
        if (fs.existsSync(this.logoPath)) {
            try {
                doc.image(this.logoPath, 50, 28, { width: 46 });
            } catch {
                /* ignore */
            }
        }
        ...
        doc.font('Helvetica')
            .fontSize(11)
            .fillColor(this.mix(accent, this.colors.white, 0.75))
            .text(title, 108, 62);
        doc.font('Helvetica')
            .fontSize(9)
            .fillColor(this.mix(accent, this.colors.white, 0.85))
            .text(String(subtitle || ''), W - 230, 44, { width: 180, align: 'right' });
        // small "codebase analyzer" eyebrow
        doc.font('Helvetica-Bold')
            .fontSize(7)
            .fillColor(this.mix(accent, this.colors.white, 0.6))
    ...
    drawPill(doc, x, y, w, h, label, bg, fg) {
        ...
        doc.font('Helvetica-Bold')
            .fontSize(7)
            .fillColor(fg)
            .text(String(label).toUpperCase(), x, y + (h - 7) / 2, {
                width: w,
                align: 'center',
    ...
    drawHeatStrip(doc, x, y, w, days, accent) {
        ...
        days.forEach((d, i) => {
            ...
            const fg =
                intensity > 0.45 || count === 0
                    ? count === 0
                        ? this.colors.muted
                        : this.colors.white
                    : this.mix(accent, this.colors.ink, 0.7);
            doc.roundedRect(cx, y, cellW, 38, 5).fill(fill);
            doc.font('Helvetica-Bold')
                .fontSize(7)
                .fillColor(count === 0 ? this.colors.muted : fg)
    ...
    drawGaugeRing(doc, cx, cy, r, value, accent, opts = {}) {
        ...
        doc.font('Helvetica-Bold')
            ...
            .text(String(Math.round(this._num(value))), cx - r, cy - (big ? 26 : 16), {
                width: r * 2,
                align: 'center',
            });
        doc.font('Helvetica-Bold')
            ...
            .text(String(label).toUpperCase(), cx - r, cy + (big ? 16 : 8), {
                width: r * 2,
                align: 'center',
                characterSpacing: 1,
            });
    ...
    drawDonut(doc, cx, cy, r, segments, opts = {}) {
        ...
        segs.forEach((seg) => {
            ...
            doc.font('Helvetica-Bold')
                ...
                .text(String(seg.label), lx + 15, ly);
            doc.font('Helvetica')
                .fontSize(8)
                .fillColor(this.colors.muted)
                .text(`${pct}%`, lx + 15, ly, { width: 120, align: 'right' });
            ly += 17;
        });
    }
    ...
    drawRadar(doc, cx, cy, r, axes, accent) {
        ...
        const dpts = axes.map((a, i) =>
            this._arcPoint(cx, cy, r * Math.max(0.04, Math.min(1, this._num(a.score) / 100)), (i / n) * 360)
        );
        ...
        axes.forEach((a, i) => {
            ...
            doc.font('Helvetica-Bold')
                ...
                .text(String(a.label).toUpperCase(), p.x - 34, p.y - 4, {
                    width: 68,
                    align: 'center',
    ...
    drawHBars(doc, x, y, w, rows, opts = {}) {
        ...
        rows.forEach((row, i) => {
            ...
            doc.font('Helvetica-Bold')
                .fontSize(8.5)
                .fillColor(this.colors.text)
                .text(String(row.label), x, ry + 2, { width: labelW - 8 });
            ...
            doc.font('Helvetica-Bold')
                .fontSize(8.5)
                .fillColor(color)
                .text(String(Math.round(this._num(row.score))), barX + barW + 8, ry + 1, { width: 28 });
        });
    ...
    drawAreaLine(doc, x, y, w, h, points, accent) {
        ...
        if (pts.length < 2) {
            doc.font('Helvetica')
                .fontSize(9)
                .fillColor(this.colors.muted)
                .text('Insufficient historical data for trend.', x, y + h / 2 - 6);
            return;
        }
        ...
        coords.forEach((c) => {
            ...
            doc.font('Helvetica-Bold')
                .fontSize(7.5)
                .fillColor(this.colors.ink)
                .text(String(Math.round(c.value)), c.x - 12, c.y - 16, { width: 24, align: 'center' });
            doc.font('Helvetica')
                .fontSize(7)
                .fillColor(this.colors.muted)
                .text(String(c.label || ''), c.x - 22, baseY + 6, { width: 44, align: 'center' });
        });
    }
    ...
    drawPage1ExecutiveDashboard(doc, data, aiInsights, subtitle) {
        ...
        kpiDefs.forEach((k, i) => {
            ...
            doc.font('Helvetica')
                .fontSize(7.5)
                .fillColor(this.colors.muted)
                .text(k.label, gx + 16, y + 14, { width: gw - 22 });
            doc.font('Helvetica-Bold')
                .fontSize(24)
                .fillColor(this.colors.ink)
                .text(String(k.val), gx + 16, y + 32);
            if (k.note)
                doc.font('Helvetica')
                    .fontSize(7)
                    .fillColor(this.colors.muted)
                    .text(k.note, gx + 16, y + 64);
        ...
        if (weeklyStats?.behavioral?.dayDistribution?.length) {
            doc.font('Helvetica-Bold')
                .fontSize(8)
                .fillColor(this.colors.textLight)
                .text('7-DAY ACTIVITY', M, y, { characterSpacing: 0.5 });
            const peak = weeklyStats.behavioral.peakDay;
            if (peak)
                doc.font('Helvetica')
                    .fontSize(8)
                    .fillColor(this.colors.muted)
                    .text(`Peak: ${peak}`, M, y, { width: CW, align: 'right' });
            y += 16;
        ...
        doc.font('Helvetica')
            .fontSize(9)
            .fillColor(this.colors.text)
            .text(String(dash.strategicFocus || 'Maintain current trajectory.'), M + cardW + 28, y + 34, {
                width: cardW - 30,
            });
    }
    ...
    drawPage2PerformanceIntelligence(doc, aiInsights, subtitle) {
        ...
        const rows = dims.map((d) => ({
            label: d.block?.status ? `${d.label}` : d.label,
            score: d.block?.score ?? 0,
            color: statusColor(d.block?.status),
        }));
        ...
        this.drawDonut(doc, M + 80, y + 86, 46, ctbSegs, {
            centerLabel: behavioral.peakDay || '',
            centerSub: 'peak day',
            legendX: M + 160,
            legendY: y + 44,
        });
        ...
        if (weeklyDelta.metrics?.length) {
            ...
            weeklyDelta.metrics.forEach((m, i) => {
                ...
                doc.font('Helvetica')
                    .fontSize(9)
                    .fillColor(this.colors.textLight)
                    .text(this._ascii(m.label), colLabel.x, ry, { width: colLabel.w });
                doc.font('Helvetica-Bold')
                    .fontSize(9)
                    .fillColor(this.colors.ink)
                    .text(this._ascii(m.current ?? '--'), colCur.x, ry, { width: colCur.w, align: 'right' });
                doc.font('Helvetica')
                    .fontSize(9)
                    .fillColor(this.colors.muted)
                    .text(this._ascii(m.estimated ?? 'N/A'), colEst.x, ry, { width: colEst.w, align: 'right' });
                const sig = m.signal || '→';
    ...
    drawPage3RepositoryIntelligence(doc, repos, aiInsights, subtitle) {
        ...
        if (topRepos.length === 0) {
            ...
            doc.font('Helvetica')
                .fontSize(11)
                .fillColor(this.colors.muted)
                .text('No repository activity recorded this week.', M + 16, y + 24);
            return;
        }

        const stateColors = {
            RAPID_GROWTH: this.colors.success,
            SECURITY_SENSITIVE: this.colors.danger,
            DORMANT: this.colors.muted,
            MAINTENANCE_HEAVY: this.colors.warning,
            HIGH_VISIBILITY: this.colors.blue,
            FEATURE_EXPANDING: this.colors.purple,
            UNSTABLE: this.colors.danger,
            STABLE: this.colors.teal,
        };
        ...
        topRepos.forEach((repo) => {
            ...
            this.drawPill(
                ...
                16,
                stateLabel,
                this.tint(stateColors[state] || accent, 0.22),
                stateColors[state] || accent
            );
            ...
            doc.font('Helvetica-Bold')
                .fontSize(13)
                .fillColor(this.colors.ink)
                .text(name, M + 16, y + 14, { width: CW - sw - 40 });
            doc.font('Helvetica')
                .fontSize(8)
                .fillColor(this.colors.muted)
                .text(
                    `${repo.commitsThisWeek || metrics.commitVelocity || 0} commits  ·  ${repo.stars || 0} stars  ·  momentum: ${metrics.momentum || 'N/A'}`,
                    M + 16,
                    y + 34
                );
            ...
            const repoSecIssues = secList.filter(
                (s) =>
            ...
            const calcVelocity = Math.min(
                100,
                Math.max(
                    20,
                    rawVelocity * 10 + (metrics.momentum === 'HIGH' ? 30 : metrics.momentum === 'MEDIUM' ? 15 : 5)
                )
            );
            ...
            const calcMaintenance =
                rawMaint > 0
                    ? Math.min(100, Math.max(15, rawMaint * 1.5))
                    : Math.max(30, 85 - repoSecIssues.length * 15);
            ...
            const meters = [
                ...
                { label: 'Maintenance', score: calcMaintenance, color: this.colors.warning },
                {
                    ...
                    score: calculatedVisRisk,
                    color:
                        calculatedVisRisk > 30
                            ? this.colors.danger
                            : calculatedVisRisk > 0
                              ? this.colors.warning
                              : this.colors.success,
            ...
            doc.font('Helvetica-Bold')
                .fontSize(7.5)
                .fillColor(this.colors.textLight)
                .text('INSIGHT', rx, y + 48, { characterSpacing: 0.5 });
            doc.font('Helvetica')
                .fontSize(8.5)
                .fillColor(this.colors.text)
                .text(insightText, rx, y + 60, { width: rw, lineGap: 1.5 });
            doc.font('Helvetica-Bold')
                .fontSize(8)
                .fillColor(accent)
                .text('» ' + this._ascii(data.suggestedAction || 'Run a dependency audit.'), rx, y + 68 + insightH, {
                    width: rw,
                });
    ...
    drawPage4AdvancedAnalytics(doc, data, history, aiInsights, subtitle) {
        ...
        if (langs.length) {
            const segs = langs.slice(0, 6).map((l, i) => ({
                label: l.name,
                value: l.percentage,
                color: this.categorical[i % this.categorical.length],
            }));
            this.drawDonut(doc, M + 86, y + 84, 50, segs, {
                centerLabel: `${langs.length}`,
                centerSub: 'languages',
                legendX: M + 180,
                legendY: y + 42,
            });
            ...
            doc.font('Helvetica')
                .fontSize(10)
                .fillColor(this.colors.muted)
                .text('No language data found.', M + 16, y + 60);
        }
        y += langH + 14;
        ...
        tiles.forEach((t, i) => {
            ...
            doc.font('Helvetica')
                .fontSize(7)
                .fillColor(this.colors.muted)
                .text(t.label, tx + 16, y + 14, { width: tw - 22 });
            doc.font('Helvetica-Bold')
                .fontSize(20)
                .fillColor(t.color)
                .text(String(t.val), tx + 16, y + 30);
        });
    }
    ...
    drawPage5SecurityIntelligence(doc, aiInsights, subtitle) {
        ...
        doc.font('Helvetica')
            .fontSize(8)
            .fillColor(this.colors.muted)
            .text('AUDIT STATUS', M + 20, y + 16, { characterSpacing: 1 });
        doc.font('Helvetica-Bold')
            .fontSize(20)
            .fillColor(statusColor)
            .text(statusText, M + 20, y + 30);
        ...
        if (sec.length === 0) {
            ...
            doc.font('Helvetica')
                .fontSize(10)
                .fillColor(this.colors.textLight)
                .text('No obvious security risks detected in recent commit activity.', M + 16, y + 22);
            return;
        }

        sec.forEach((s) => {
            const evH = s.evidence ? Math.max(12, doc.heightOfString(String(s.evidence), { width: CW - 200 })) : 0;
            const fixH = s.suggestion
                ? Math.max(12, doc.heightOfString(String(s.suggestion), { width: CW - 130 }))
                : 12;
            ...
            doc.font('Helvetica-Bold')
                .fontSize(10.5)
                .fillColor(this.colors.ink)
                .text(String(s.issue || 'Finding'), M + 16, y + 38, { width: CW - 32 });
            if (s.affectedArea)
                doc.font('Helvetica')
                    .fontSize(8)
                    .fillColor(this.colors.muted)
                    .text(`Area: ${s.affectedArea}`, M + 16, y + 54);
            let ey = y + 68;
            if (s.evidence) {
                doc.font('Helvetica-Bold')
                    .fontSize(8)
                    .fillColor(this.colors.textLight)
                    .text('Evidence', M + 16, ey);
                doc.font('Helvetica')
                    .fontSize(8.5)
                    .fillColor(this.colors.text)
                    .text(String(s.evidence), M + 80, ey, { width: CW - 100 });
                ey += evH + 4;
            }
            doc.font('Helvetica-Bold')
                .fontSize(8)
            ...
            doc.font('Helvetica')
                .fontSize(8.5)
                .fillColor(this.colors.text)
                .text(String(s.suggestion || 'Review and remediate.'), M + 80, ey, { width: CW - 100 });
    ...
    drawPage6WeeklyNarrative(doc, aiInsights, data, subtitle) {
        ...
        this.drawGaugeRing(doc, M + 120, y + 104, 64, globalScore.total ?? 0, accent, {
            label: 'overall',
            thickness: 15,
        });

        const bd = globalScore.breakdown || {};
        const bdAxes = Object.keys(bd)
            .slice(0, 6)
            .map((k) => ({ label: k.slice(0, 8), score: bd[k] ?? 0 }));
        if (bdAxes.length >= 3) {
            this.drawRadar(doc, M + CW - 150, y + 100, 60, bdAxes, this.colors.purple);
        } else {
            const rows = Object.keys(bd).map((k, i) => ({
                label: k,
                score: bd[k] ?? 0,
                color: this.categorical[i % this.categorical.length],
            }));
        ...
        const narrative = String(
            aiInsights?.narrative ||
                'Steady progression this week. Maintain consistent commits and continue improving code quality.'
        );
        const narrW = CW - 64;
        ...
        doc.font('Helvetica-Bold')
            .fontSize(30)
            .fillColor(this.tint(this.colors.purple, 0.5))
            .text('“', M + 18, y + 26);
        doc.font('Helvetica')
            .fontSize(10)
            .fillColor(this.colors.text)
            .text(narrative, M + 42, y + 46, { width: narrW, lineGap: 4 });
        y += narrH + 14;
        ...
        if (badges.length) {
            doc.font('Helvetica-Bold')
                .fontSize(8)
                .fillColor(this.colors.textLight)
                .text('EARNED BADGES', M, y, { characterSpacing: 0.5 });
            y += 24;
    ...
    renderReportDocument(input) {
        ...
        return new Promise((resolve, reject) => {
            ...
            const drawFooter = (d, accent) => {
                ...
                d.moveTo(50, fy - 8)
                    .lineTo(d.page.width - 50, fy - 8)
                    .lineWidth(1)
                    .strokeColor(this.tint(accent, 0.5))
                    .stroke();
                ...
                d.font('Helvetica')
                    ...
                    .fillColor(this.colors.muted)
                    .text(
                        'Confidential Weekly Performance Analysis  ·  Generated by DevTrack Codebase Intelligence Engine',
                        50,
                        fy,
                        { align: 'center', width: d.page.width - 100 }
                    );
            };
            ...
            const impactScore = Math.min(
                99,
                Math.round((insights.rank?.score || 0) / 10 + activityBonus + (streak > 5 ? 10 : 0))
            );
            ...
            const renderData = {
                ...
                insights,
                repos: repos || [],
                activeRepos: Array.isArray(activeRepos)
                    ? activeRepos
                    : Array.from((activeRepos || new Map()).values?.() || []),
            ...
            try {
                ...
                doc.addPage();
                this.drawPage2PerformanceIntelligence(doc, aiInsights, sub);
                doc.addPage();
                this.drawPage3RepositoryIntelligence(
                    doc,
                    renderData.activeRepos.length > 0 ? renderData.activeRepos : renderData.repos,
                    aiInsights,
                    sub
                );
                doc.addPage();
                this.drawPage4AdvancedAnalytics(doc, renderData, history, aiInsights, sub);
                doc.addPage();
                this.drawPage5SecurityIntelligence(doc, aiInsights, sub);
                doc.addPage();
                this.drawPage6WeeklyNarrative(doc, aiInsights, renderData, sub);
            } catch (err) {
                console.error('PDF Rendering Error:', err);
                doc.font('Helvetica-Bold')
                    .fontSize(14)
                    .fillColor(this.colors.danger)
                    .text('Error Rendering PDF', 50, 150);
    ...
    async generatePDFReport(userId, preExistingData = null) {
        ...
        if (preExistingData) {
            ...
            recentActivity = {
                totalEvents: (preExistingData.stats?.totalCommits || 0) + (preExistingData.stats?.totalPRs || 0),
            };
            ...
            try {
                ...
                console.log(
                    `📊 Weekly stats: ${weeklyStats?.weekly?.commits} commits, ${weeklyStats?.weekly?.activeDays}/7 active days`
                );
            } catch (e) {
        ...
        const subtitle = preExistingData
            ? new Date(preExistingData.createdAt).toLocaleDateString('en-US', {
                  weekday: 'long',
                  month: 'short',
                  day: 'numeric',
                  year: 'numeric',
    ...
    async sendWeeklyReportEmail(userId, options = {}) {
        try {
            ...
            if (skipEmail) {
                console.log(`Skipping email dispatch (options.skipEmail is true) for user ${userId}`);
                return { success: true, message: 'Report generated and saved (email skipped)' };
            }
